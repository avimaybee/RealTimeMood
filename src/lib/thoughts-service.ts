...
export async function incrementLike(quoteId: string): Promise<void> {
  const db = getDb();
  if (!quoteId) {
    console.error("incrementLike called with invalid quoteId:", quoteId);
  ...
  try {
    ...
    console.error(`Error incrementing like count for quoteId: ${quoteId}`, error);
...
export async function decrementLike(quoteId: string): Promise<void> {
  const db = getDb();
  if (!quoteId) {
    console.error("decrementLike called with invalid quoteId:", quoteId);
    throw new Error("Invalid quote ID provided.");
  }
  const quoteRef = doc(db, 'communityQuotes', quoteId);
  try {
    ...
    console.error(`Error decrementing like count for quoteId: ${quoteId}`, error);
    // Re-throw the error to be handled by the calling component (e.g., to show a toast)
    throw new Error("Could not update like count.");
  }
}
...
/**
 * Clears the 'typing' status from Firestore.
 * @param userId The UID of the user.
 */
