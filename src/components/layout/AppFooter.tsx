...
const AppFooter: React.FC<AppFooterProps> = ({ isMenuOpen, setIsMenuOpen, setIsAmbientMode }) => {
  const { contributionCount, isCollectiveShifting, currentMood } = useMood();
  const moodColor = `hsl(${currentMood.hue}, ${currentMood.saturation}%, ${currentMood.lightness}%)`;
        ...
        <div className="flex-shrink-0 w-full flex items-center justify-between h-[52px] px-4">
          <div className="text-sm md:text-base opacity-90 font-medium">
            <motion.span
              ...
              style={{ textShadow: `0 0 8px ${moodColor}40` }}
            >0</motion.span> moods shared
          </div>
            ...
            >
              <div className="w-full flex flex-col items-start">
                ...
                <div className="w-full px-2 pt-2 pb-1">
                  <div
                    className="flex items-center justify-center gap-x-2 p-1.5 rounded-xl transition-all duration-300"
                    style={{
                      background: `linear-gradient(135deg, ${moodColor}15, transparent)`,
                      boxShadow: `inset 0 0 20px ${moodColor}10`
                    }}
                  >
                    <Button asChild variant="ghost" size="icon" className="rounded-full h-9 w-9 hover:bg-white/10 transition-all duration-300 hover:scale-110 hover:shadow-lg" style={{ '--hover-glow': moodColor } as React.CSSProperties}>
                      <Link href="https://instagram.com/avimaybe" target="_blank" rel="noopener noreferrer" aria-label="Instagram">
                        <Instagram className="h-5 w-5 transition-all duration-300 group-hover:drop-shadow-lg" strokeWidth={isIos ? 1.5 : 2} />
                      </Link>
                    </Button>
                    <Button asChild variant="ghost" size="icon" className="rounded-full h-9 w-9 hover:bg-white/10 transition-all duration-300 hover:scale-110 hover:shadow-lg">
                      <Link href="https://www.snapchat.com/add/avimaybe" target="_blank" rel="noopener noreferrer" aria-label="Snapchat">
                        <Ghost className="h-5 w-5 transition-all duration-300" strokeWidth={isIos ? 1.5 : 2} />
                      </Link>
                    </Button>
                    <Button asChild variant="ghost" size="icon" className="rounded-full h-9 w-9 hover:bg-white/10 transition-all duration-300 hover:scale-110 hover:shadow-lg">
                      <Link href="https://github.com/avimaybee" target="_blank" rel="noopener noreferrer" aria-label="GitHub">
                        <Github className="h-5 w-5 transition-all duration-300" strokeWidth={isIos ? 1.5 : 2} />
                      </Link>
                    </Button>
                    <div
                      className="w-px h-6 mx-1 transition-all duration-500"
                      style={{ background: `linear-gradient(to bottom, transparent, ${moodColor}50, transparent)` }}
                    />
                    <Button variant="ghost" size="icon" className="rounded-full h-9 w-9 hover:bg-white/10 transition-all duration-300 hover:scale-110 hover:shadow-lg" onClick={handleShare} aria-label="Share or copy link">
                      <Share2 className="h-5 w-5 transition-all duration-300" strokeWidth={isIos ? 1.5 : 2} />
                    </Button>
                  </div>
                </div>
